...
import { AUTH_TOKEN_KEY, API_BASE_URL } from '@/services/authService'
  ...
  useEffect(() => {
    ...
    try {
     ...
     if (!response.ok) {
          ...
          throw new Error(
               errorData.error || 'Retrieval failed.'
          )
     }
     console.log(response.body)
    } catch (error) {
          console.error('Store Item retrieval error:', error)
          throw new Error('An error occurred during retrieval. Please try again.');
    }
  }, []);
