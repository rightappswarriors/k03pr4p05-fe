...
export default function usePOS () {
  ...
  const addToCart = (item: Item, quantity: number = 1) => {
    setCartItems((prev) => {
      const existingItem = prev.find((cartItem) => cartItem.data.id === item.data.id);
      if (existingItem) {
        return prev.map((cartItem) =>
          cartItem.data.id === item.data.id
  ...
  const updateQuantity = (id: string, quantity: number) => {
    ...
    setCartItems((prev) =>
      prev.map((item) => (item.data.id === id ? { ...item, quantity } : item))
    );
  };

  const removeFromCart = (id: string) => {
    setCartItems((prev) => prev.filter((item) => item.data.id !== id));
  };
  ...
  const filteredItems = items.filter((item) => {
    ...
    const matchesSearch =
      normalizedSearch === '' ||
      item.data.name.toLowerCase().includes(normalizedSearch) ||
      item.data.barcode?.includes(normalizedSearch);

    const matchesCategory =
      !selectedCategory || item.data.categoryId === selectedCategory;
