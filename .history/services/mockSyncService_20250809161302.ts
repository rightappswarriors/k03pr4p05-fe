...
export class MockSyncService {
  ...
  static async startBackgroundSync(): Promise<void> {
    ...
    const pending = await this StorageService.getPendingOrders()
    if (!pending || pending.length === 0){ 
      console.log('No pending orders - skyping background sync')
      return;
    }
    ...
    setInterval(async () => {
      if (!this.syncInProgress) {
        ...
        console.log('syncing for 10 secs')
        eventBus.emit('orderSynced');
      }
    }, 40000); // Check every 10 seconds for demo
  }
