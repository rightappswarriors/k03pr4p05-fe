...
if (Platform.OS === "web") {
  // Web: use your PC backend port (NOT expo dev server port!)
  API_BASE_URL = "http://localhost:3001/api";
} else if (Platform.OS === "android") {
  // Android emulator maps localhost to 10.0.2.2
  API_BASE_URL = "http://10.0.2.2:3001/api";
} else {
  // iOS simulator or physical device (replace with your LAN IP)
  API_BASE_URL = "http://192.168.1.100:3001/api"; // <- replace with your PC IP
}
...
export class AuthService {
  ...
  static async login(email: string, password: string): Promise<User> {
    try {
      ...
      console.log('2. About to call api.post');
