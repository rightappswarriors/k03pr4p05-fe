...
export class AuthService {
  ...
  static async login(email: string, password: string): Promise<User> {
    try {
      ...
      const userData =await secureStorage.setItemAsync(USER_DATA_KEY, JSON.stringify(user));
      await secureStorage.setItemAsync(REFRESH_TOKEN_KEY, refresh_token);
      const accessToken = await secureStorage.getItemAsync(AUTH_TOKEN_KEY);
      const userCookie = await secureStorage.getItemAsync(USER_DATA_KEY);
      console.log('Get Access token from cookie, after login', accessToken)
      console.log('6. Login and token storage successful.');
      console.log('Token:', token);
      console.log('Refresh Token:', refresh_token);
      console.log('User Details:', userData);
      console.log('User Cookie Data:', userCookie);
      const user_data = await this.getCurrentUser() 
      return user;
    } catch (error) {
