...
import { Transaction, Store, User, SyncLog, AuthState } from '@/types';
...
export class ApiService {
  ...
  static async syncOrders(orders: Transaction[]): Promise<{ success: boolean; syncedOrderIds: string[]; errors: string[] }> {
    const deviceInfo = await DeviceService.getDeviceInfo();
    const syncStartTime = Date.now();
    
    try {
      const response = await this.makeRequest('/transaction/sync', {
        method: 'POST',
        body: JSON.stringify({ 
          orders,
          deviceInfo,
      ...
      await this.logSync({
        ...
        deviceId: deviceInfo.deviceId, // ✅ Add this
        deviceInfo,
