...
export function CartSidebar({
  ...
  const total = items.reduce((sum, item) => sum + (item.data.price * item.quantity), 0);
      ...
      >
        ...
        <ScrollView style={[styles.itemsList, { backgroundColor: colors.background, borderBottomColor: colors.border }]} showsVerticalScrollIndicator={false}>
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <View key={item.data.id} style={[styles.cartItem, { borderColor: colors.border }]}>
                <Image source={{ uri: item.data.image }} style={styles.itemImage} />
                <View style={styles.itemDetails}>
                  <Text style={[styles.itemName, { color: colors.text }]} numberOfLines={2}>{item.data.name}</Text>
                  <Text style={[styles.itemPrice, { color: colors.text }]}>${item.data.price.toFixed(2)}</Text>
                  <View style={styles.quantityContainer}>
                    <TouchableOpacity
                      onPress={() => onUpdateQuantity(item.data.id, item.quantity - 1)}
                    ...
                    <TextInput
                      ...
                      onChangeText={(text) => {
                        ...
                        onUpdateQuantity(item.data.id, num);
                      }}
                ...
                <View style={styles.itemRight}>
                  <Text style={[styles.subtotal, { color: colors.text }]}>${(item.data.price * item.quantity).toFixed(2)}</Text>
                  <TouchableOpacity
                    onPress={() => onRemoveItem(item.data.id)}
