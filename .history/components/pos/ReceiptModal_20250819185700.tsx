  ...
  const handlePrintReceipt = async () => {
    ...
    const receiptData = {
      ...
      items: items.map(data => ({
        ...
        subtotal: data.item.price * data.quantity,
        ...
        <View style={[styles.modal, { backgroundColor: colors.card}]}>
          ...
          <ScrollView style={[styles.content, { borderColor: colors.border}]} showsVerticalScrollIndicator={false}>
            <View style={[styles.section]}>
              ...
              {items.map((data) => (
                <View key={data.item.id} style={styles.itemRow}>
                  <View style={styles.itemInfo}>
                    <Text style={[styles.itemName, { color: colors.text}]}>{data.item.name}</Text>
                    <Text style={[styles.itemDetails, { color: colors.textSecondary}]}>
                      {data.quantity} × ₱{data.item.price.toFixed(2)}
                    </Text>
                  </View>
