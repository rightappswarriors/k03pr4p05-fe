...
export function CartSidebar({
  ...
  const total = items.reduce((sum, data) => sum + (data.item.price * data.quantity), 0);
      ...
      >
        ...
        <ScrollView style={[styles.itemsList, { backgroundColor: colors.background, borderBottomColor: colors.border }]} showsVerticalScrollIndicator={false}>
          {items.length === 0 ? (
            ...
            items.map((data) => (
              <View key={data.item.id} style={[styles.cartItem, { borderColor: colors.border }]}>
                <Image source={{ uri: data.item.image }} style={styles.itemImage} />
                <View style={styles.itemDetails}>
                  <Text style={[styles.itemName, { color: colors.text }]} numberOfLines={2}>{data.item.name}</Text>
                  <Text style={[styles.itemPrice, { color: colors.text }]}>${data.item.price.toFixed(2)}</Text>
                  <View style={styles.quantityContainer}>
                    <TouchableOpacity
                      onPress={() => onUpdateQuantity(data.item.id, data.quantity - 1)}
                    ...
                    <TextInput
                      ...
                      onChangeText={(text) => {
                        ...
                        onUpdateQuantity(data.item.id, num);
                      }}
                ...
                <View style={styles.itemRight}>
                  <Text style={[styles.subtotal, { color: colors.text }]}>${(data.item.price * data.quantity).toFixed(2)}</Text>
                  <TouchableOpacity
