...
export class AuthService {
  ...
  static async authenticateWithBiometric(): Promise<boolean> {
    try {
      ...
      console.log('authenticateWithBiometric result:', result);
  ...
  static async loginWithBiometric(): Promise<User | null> {
    ...
    console.log('loginWithBiometric isEnabled',isEnabled)
    if (!isEnabled) return null;

    console.log('Authenticate user through biometric after checking...');

    const isAuthenticated = await this.authenticateWithBiometric();
    if (!isAuthenticated) return null;

    console.log('Biometric authenticated. Logging in default user... isAuthenticated?', isAuthenticated);
    ...
    try {
      ...
      console.log('User auto-login after biometric:', user);
