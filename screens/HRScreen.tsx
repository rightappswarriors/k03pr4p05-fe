...
import {
  getPendingProcurementAgents,
  getOrganizationAgentDetails,
  getOrganizationAgents,
  updateOrganizationAgent,
  rejectApprovedOrganizationAgent,
  rejectPendingOrganizationAgent,
  approveOrganizationAgent,
} from '@/services/procurementAgentService';
import { PendingAgentModal } from '@/screens/hr/PendingAgentModal';
import { OrganizationAgentModal } from '@/screens/hr/OrganizationAgentModal';
...
import { InvitationMethod } from '@/services/invitationService'
...
type HrSectionTab = 'employees' | 'procurementAgents' | 'invitations' | 'pendingRequests'
...
// ─── Procurement Agent Component ─────────────────────────────────────────────
function ProcurementAgentTable({ agents, onView, onEdit, onReject }: {
  agents: OrganizationAgent[]
  onView?: (agent: OrganizationAgent) => void
  onEdit?: (agent: OrganizationAgent) => void
  onReject?: (agent: OrganizationAgent) => void
}) {
  const { colors } = useTheme()
  const { width } = useWindowDimensions()
  const isWide = width >= 1400
  ...
  const handleRowPress = (agent: OrganizationAgent) => {
    onView?.(agent)
  }
  ...
  return (
    <View style={{ borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: colors.border }}>
      <View style={{ flexDirection: 'row', paddingHorizontal: 16, paddingVertical: 10, backgroundColor: colors.background, borderBottomWidth: 1, borderBottomColor: colors.border }}>
        <View style={{ flex: 3, minWidth: 180 }}><Text style={colHeader}>Name</Text></View>
        {isWide && <View style={{ flex: 2, minWidth: 180 }}><Text style={colHeader}>Email</Text></View>}
        <View style={{ flex: 2, minWidth: 120 }}><Text style={colHeader}>Position</Text></View>
        <View style={{ width: 100 }}><Text style={colHeader}>Status</Text></View>
        <View style={{ width: 100 }}><Text style={colHeader}>Joined</Text></View>
        <View style={{ width: 80 }} />
      </View>
      {agents.length === 0 ? (
        <View style={{ padding: 40, alignItems: 'center' }}>
          <Text style={{ color: colors.textSecondary, fontSize: 14 }}>No approved procurement agents</Text>
        </View>
      ) : (
        agents.map((agent, i) => (
          <TouchableOpacity
          ...
          >
            <View style={{ flex: 3, minWidth: 180, flexDirection: 'row', alignItems: 'center', gap: 10 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={cell} numberOfLines={1}>{agent.fullname || agent.email || 'Agent'}</Text>
                {!isWide && <Text style={cellSub} numberOfLines={1}>{agent.email ?? '—'}</Text>}
              </View>
            </View>
            {isWide && <View style={{ flex: 2, minWidth: 180 }}><Text style={cellSub} numberOfLines={1}>{agent.email ?? '—'}</Text></View>}
            <View style={{ flex: 2, minWidth: 120 }}>
              {agent.positionName ? (
                <Badge label={agent.positionName} tone="#0EA5E9" size="xs" />
              ) : (
                <Text style={{ fontSize: 12, color: colors.textSecondary }}>—</Text>
              )}
            </View>
            <View style={{ width: 100 }}>
              <Badge label={agent.verificationStatus} tone={agent.verificationStatus === 'APPROVED' ? '#16A34A' : '#F59E0B'} size="xs" />
            </View>
            <View style={{ width: 100 }}>
              <Text style={cellSub}>{agent.createdAt ? formatDate(agent.createdAt) : '—'}</Text>
            </View>
            <View style={{ width: 80, flexDirection: 'row', gap: 4, alignItems: 'center' }}>
              {onView && (
                <TouchableOpacity onPress={() => onView(agent)} style={{ padding: 4 }}>
                  <Eye size={16} color={colors.primary} />
                </TouchableOpacity>
              )}
              {onEdit && (
                <TouchableOpacity onPress={() => onEdit(agent)} style={{ padding: 4 }}>
                  <Edit3 size={16} color={colors.textSecondary} />
                </TouchableOpacity>
              )}
              {onReject && (
                <TouchableOpacity onPress={() => onReject(agent)} style={{ padding: 4 }}>
...
function InvitationTable({
  ...
  const isWide = screenWidth >= 1400

  // Single context menu state (only one menu open at a time)
  const menuState = useContextMenu()
  const [activeInvitation, setActiveInvitation] = useState<InvitationRow | null>(null)
  const buttonRefs = useRef<Record<string, View | null>>({})

  // Get context menu items for current invitation
  const menuItems = useMemo((): ContextMenuItem[] => {
    if (!activeInvitation) return []

    const items: ContextMenuItem[] = []

    if (activeInvitation.link && onCopyLink) {
      items.push({
        key: 'copy-link',
        label: 'Copy Link',
    ...
    if (activeInvitation.code && onCopyCode) {
      items.push({
        key: 'copy-code',
        label: 'Copy Code',
        icon: <Copy size={14} color={colors.text} />,
    ...
    if (activeInvitation.status === 'PENDING') {
      if (onResend) {
        items.push({
          ...
          icon: <Send size={14} color={colors.text} />,
          color: colors.text,
        })
      }
      if (onExtend) {
        items.push({
          ...
          icon: <Calendar size={14} color={colors.text} />,
          color: colors.text,
        })
      }
    }
    ...
    // Destructive actions
    if (activeInvitation.status === 'PENDING' && onRevoke) {
      items.push({
        key: 'revoke',
    ...
    return items
  }, [activeInvitation, onCopyLink, onCopyCode, onResend, onExtend, onRevoke, onDelete, colors.text])
  ...
  const handleMenuSelect = useCallback((key: string) => {
    ...
    switch (key) {
      case 'copy-link':
        onCopyLink?.(activeInvitation.link!)
        break
      case 'copy-code':
        onCopyCode?.(activeInvitation.code!)
        break
      case 'resend':
        onResend?.(activeInvitation.id)
        break
      case 'extend':
        onExtend?.(activeInvitation.id)
        break
      case 'revoke':
        onRevoke?.(activeInvitation.id)
        break
      case 'delete':
        onDelete?.(activeInvitation.id)
        break
    }
  }, [activeInvitation, onCopyLink, onCopyCode, onResend, onExtend, onRevoke, onDelete])

  // Measure button position and open menu
  const openMenuForInvitation = useCallback((inv: InvitationRow, index: number) => {
    const ref = buttonRefs.current[inv.id]
    if (ref) {
      // @ts-ignore - measure is available on native View
      ref.measure((fx: number, fy: number, w: number, h: number, px: number, py: number) => {
        menuState.openMenu({ x: px, y: py, width: w, height: h })
        setActiveInvitation(inv)
      })
    }
  }, [menuState])

  // Close menu when it's closed
  useEffect(() => {
  ...
  return (
    <View style={{ borderRadius: 12, borderWidth: 1, borderColor: colors.border }}>
      <View style={{ flexDirection: 'row', paddingHorizontal: 16, paddingVertical: 10, backgroundColor: colors.background, borderBottomWidth: 1, borderBottomColor: colors.border }}>
        <View style={{ flex: 2, minWidth: 180 }}><Text style={colHeader}>Email / Code</Text></View>
        {isWide && <View style={{ flex: 2, minWidth: 180 }}><Text style={colHeader}>Link</Text></View>}
        <View style={{ flex: 1, minWidth: 120 }}><Text style={colHeader}>Position</Text></View>
        <View style={{ width: 100 }}><Text style={colHeader}>Status</Text></View>
        <View style={{ width: 120 }}><Text style={colHeader}>Expires</Text></View>
        <View style={{ width: 48 }} />
      </View>
      {invitations.length === 0 ? (
        <View style={{ padding: 40, alignItems: 'center' }}>
          <Text style={{ color: colors.textSecondary, fontSize: 14 }}>No invitations found</Text>
        </View>
      ) : (
        invitations.map((inv, i) => (
          <View key={inv.id} style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10, borderTopWidth: i === 0 ? 0 : 1, borderTopColor: colors.border }}>
            <View style={{ flex: 2, minWidth: 180 }}>
              <Text style={cell} numberOfLines={1}>{inv.email || inv.code}</Text>
              {inv.code && <Text style={cellSub} numberOfLines={1}>Code: {inv.code}</Text>}
            </View>
            {isWide && <View style={{ flex: 2, minWidth: 180 }}><Text style={cellSub} numberOfLines={1}>{inv.link ?? '—'}</Text></View>}
            <View style={{ flex: 1, minWidth: 120 }}>
              {inv.position ? (
                <Badge label={inv.position.name} tone="#0EA5E9" size="xs" />
              ) : (
                <Text style={{ fontSize: 12, color: colors.textSecondary }}>—</Text>
              )}
            </View>
            <View style={{ width: 100 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                <StatusDot active={inv.status === 'ACCEPTED'} />
                <Text style={{ fontSize: 12, fontWeight: '600', color: inv.status === 'ACCEPTED' ? '#16A34A' : '#6B7280' }}>{inv.status}</Text>
              </View>
            </View>
            <View style={{ width: 120 }}>
              <Text style={cellSub}>{inv.expiresAt ? formatDate(inv.expiresAt) : 'Never'}</Text>
            </View>
            <View style={{ width: 48, alignItems: 'center' }}>
              <TouchableOpacity
                ref={ref => { buttonRefs.current[inv.id] = ref }}
                onPress={() => openMenuForInvitation(inv, i)}
                style={{ width: 32, height: 32, borderRadius: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.background }}
              >
                <MoreHorizontal size={15} color={colors.textSecondary} />
              </TouchableOpacity>
            </View>
...
function PendingRequestsTable({ requests, onApprove, onReject, onViewDetails }: {
  ...
  return (
    <View style={{ borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: colors.border }}>
      <View style={{ flexDirection: 'row', paddingHorizontal: 16, paddingVertical: 10, backgroundColor: colors.background, borderBottomWidth: 1, borderBottomColor: colors.border }}>
        <View style={{ flex: 2, minWidth: 180 }}><Text style={colHeader}>Agent</Text></View>
        <View style={{ flex: 2, minWidth: 180 }}><Text style={colHeader}>Email</Text></View>
        <View style={{ flex: 2, minWidth: 120 }}><Text style={colHeader}>Message</Text></View>
        <View style={{ width: 100 }}><Text style={colHeader}>Status</Text></View>
        <View style={{ width: 100 }}><Text style={colHeader}>Actions</Text></View>
      </View>
      {requests.length === 0 ? (
        <View style={{ padding: 40, alignItems: 'center' }}>
          <Text style={{ color: colors.textSecondary, fontSize: 14 }}>No pending requests found</Text>
        </View>
      ) : (
        requests.map((req, i) => (
          <TouchableOpacity
            key={req.id}
            onPress={() => onViewDetails?.(req.agentId ?? req.agent?.id ?? '')}
            style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10, borderTopWidth: i === 0 ? 0 : 1, borderTopColor: colors.border }}
            activeOpacity={0.7}
          >
            <View style={{ flex: 2, minWidth: 180 }}>
              <Text style={cell} numberOfLines={1}>{req.agent?.fullname || '—'}</Text>
            </View>
            <View style={{ flex: 2, minWidth: 180 }}>
              <Text style={cellSub} numberOfLines={1}>{req.agent?.email || '—'}</Text>
            </View>
            <View style={{ flex: 2, minWidth: 120 }}>
              <Text style={cellSub} numberOfLines={1}>{req.message || '—'}</Text>
            </View>
            <View style={{ width: 100 }}>
              <Badge label={req.status} tone={req.status === 'PENDING' ? '#F59E0B' : req.status === 'APPROVED' ? '#16A34A' : '#DC2626'} size="xs" />
            </View>
            <View style={{ width: 100, flexDirection: 'row', gap: 8 }}>
              {onViewDetails && (
                <TouchableOpacity onPress={() => onViewDetails(req.agentId ?? req.agent?.id ?? '')} style={{ padding: 4 }}>
                  <Eye size={16} color={colors.primary} />
                </TouchableOpacity>
              )}
              {req.status === 'PENDING' && onApprove && (
                <TouchableOpacity onPress={() => onApprove(req.id)} style={{ padding: 4 }}>
                  <UserCheck size={16} color="#16A34A" />
                </TouchableOpacity>
              )}
              {req.status === 'PENDING' && onReject && (
                <TouchableOpacity onPress={() => onReject(req.id)} style={{ padding: 4 }}>
...
function InvitationDrawer({ visible, positions, onClose, onGenerated }: InvitationDrawerProps) {
  const { colors } = useTheme()
  const { width } = useWindowDimensions()
  const isDesktop = width >= 1024
  ...
  const [errors, setErrors] = useState<{ email?: string; position?: string; expiration?: string }>({})

  // Validation
  const validateForm = useCallback(() => {
    const newErrors: { email?: string; position?: string; expiration?: string } = {}

    if (method === 'EMAIL' && !email) {
      newErrors.email = 'Email is required'
    } else if (method === 'EMAIL' && email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
  ...
  }, [method, email, positionId, expiresInDays])

  const isValid = Object.keys(errors).length === 0 && !!positionId && expiresInDays !== -1
  ...
  useEffect(() => {
    if (!visible) {
      ...
      setEmail('')
      setPositionId(null)
      setExpiresInDays(1440) // Default to 1 day in minutes
      setCustomMessage('')
      setErrors({})
    }
  }, [visible])
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      // Convert minutes to days for backend (except -1 for "Never")
      const daysValue = expiresInDays === -1 ? -1 : expiresInDays / 1440
      const result = method === 'EMAIL'
        ? await HrService.inviteProcurementAgent({ email, positionId: positionId ?? undefined, expiresInDays: daysValue })
        : await HrService.generateInvitation({ positionId: positionId ?? undefined, expiresInDays: daysValue })
      onGenerated(result)
      onClose()
    } catch (e: any) {
      Alert.alert('Error', e?.message ?? 'Failed to generate invitation')
    } finally {
      setSubmitting(false)
    }
  }

  const slideX = useRef(new Animated.Value(isDesktop ? 0 : width)).current
  const fadeOverlay = useRef(new Animated.Value(0)).current

  useEffect(() => {
    if (visible) {
      Animated.parallel([
        Animated.spring(slideX, { toValue: 0, useNativeDriver: true, speed: 30 }),
        Animated.timing(fadeOverlay, { toValue: 1, duration: 200, useNativeDriver: true }),
      ]).start()
    } else {
      Animated.parallel([
        Animated.timing(slideX, { toValue: isDesktop ? 0 : width, duration: 220, useNativeDriver: true }),
        Animated.timing(fadeOverlay, { toValue: 0, duration: 180, useNativeDriver: true }),
      ]).start()
    }
  }, [visible, isDesktop])
        ...
        <Animated.View style={{ width: drawerWidth, backgroundColor: colors.surface, borderLeftWidth: isDesktop ? 1 : 0, borderLeftColor: colors.border, transform: [{ translateX: slideX }] }}>
          <View style={{ flex: 1 }}>
            <View style={{ padding: 18, borderBottomWidth: 1, borderBottomColor: colors.border, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: colors.text, fontSize: 17, fontWeight: '900' }}>Generate Procurement Agent Invitation</Text>
                <Text style={{ color: colors.textSecondary, fontSize: 12, marginTop: 1 }}>Invite a Procurement Agent to join your organization</Text>
              </View>
              <TouchableOpacity onPress={onClose}><X size={18} color={colors.textSecondary} /></TouchableOpacity>
            </View>

            <ScrollView contentContainerStyle={{ padding: 18, gap: 16 }}>
              {/* Section 1: Invitation Method */}
              <View style={{ gap: 8 }}>
                <Text style={{ fontSize: 12, fontWeight: '700', color: colors.textSecondary, textTransform: 'uppercase' }}>Invitation Method</Text>
                <View style={{ gap: 6 }}>
                  {[
                    { key: 'EMAIL', label: 'Email Invitation', desc: 'Send an invitation directly to the recipient.' },
                    { key: 'LINK', label: 'Invitation Link', desc: 'Generate a secure shareable link.' },
                    { key: 'CODE', label: 'Invitation Code', desc: 'Generate a one-time invitation code.' },
                  ].map(opt => (
                    <TouchableOpacity key={opt.key} onPress={() => setMethod(opt.key as InvitationMethod)} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 12, borderRadius: 10, borderWidth: 1, borderColor: method === opt.key ? colors.primary : colors.border, backgroundColor: method === opt.key ? `${colors.primary}0A` : colors.surface }}>
                      <View style={{ width: 16, height: 16, borderRadius: 8, borderWidth: 2, borderColor: method === opt.key ? colors.primary : colors.border, alignItems: 'center', justifyContent: 'center' }}>
                        {method === opt.key && <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: colors.primary }} />}
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 13, fontWeight: '700', color: colors.text }}>{opt.label}</Text>
              ...
              {/* Section 2: Email (if Email selected) */}
              {method === 'EMAIL' && (
                <View style={{ gap: 6 }}>
                  <Text style={{ fontSize: 12, fontWeight: '700', color: colors.textSecondary }}>Email Address *</Text>
                  <TextInput
                    ...
                    autoCapitalize="none"
                    style={{ borderWidth: 1, borderColor: errors.email ? '#DC2626' : colors.border, borderRadius: 10, padding: 12, color: colors.text, fontSize: 14, backgroundColor: colors.background }}
                  />
                  {errors.email && <Text style={{ fontSize: 11, color: '#DC2626' }}>{errors.email}</Text>}
                </View>
              )}

              {/* Section 3: Position */}
              <View style={{ gap: 6 }}>
                <Text style={{ fontSize: 12, fontWeight: '700', color: colors.textSecondary }}>Position *</Text>
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  <View style={{ flexDirection: 'row', gap: 6 }}>
                    {positions.map(p => (
                      <TouchableOpacity key={p.id} onPress={() => setPositionId(positionId === p.id ? null : p.id)} style={{ borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6, borderWidth: 1, borderColor: errors.position ? '#DC2626' : (positionId === p.id ? colors.primary : colors.border), backgroundColor: positionId === p.id ? `${colors.primary}14` : colors.surface }}>
                        <Text style={{ fontSize: 12, fontWeight: '700', color: positionId === p.id ? colors.primary : colors.textSecondary }}>{p.name}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                </ScrollView>
                {errors.position && <Text style={{ fontSize: 11, color: '#DC2626' }}>{errors.position}</Text>}
              </View>

              {/* Section 4: Expiration */}
              <View style={{ gap: 6 }}>
                <Text style={{ fontSize: 12, fontWeight: '700', color: colors.textSecondary }}>Expiration *</Text>
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  <View style={{ flexDirection: 'row', gap: 6 }}>
                    {[
                      ...
                      <TouchableOpacity key={opt.label} onPress={() => setExpiresInDays(opt.value)} style={{ borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6, borderWidth: 1, borderColor: errors.expiration ? '#DC2626' : (expiresInDays === opt.value ? colors.primary : colors.border), backgroundColor: expiresInDays === opt.value ? `${colors.primary}14` : colors.surface }}>
                        <Text style={{ fontSize: 12, fontWeight: '700', color: expiresInDays === opt.value ? colors.primary : colors.textSecondary }}>{opt.label}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                </ScrollView>
                {errors.expiration && <Text style={{ fontSize: 11, color: '#DC2626' }}>{errors.expiration}</Text>}
              </View>

              {/* Section 5: Custom Message (if Email or Link) */}
              {(method === 'EMAIL' || method === 'LINK') && (
                <View style={{ gap: 6 }}>
                  <Text style={{ fontSize: 12, fontWeight: '700', color: colors.textSecondary }}>Custom Message</Text>
                  <TextInput
                    value={customMessage}
                    ...
                    placeholderTextColor={colors.textSecondary}
                    multiline
                    style={{ borderWidth: 1, borderColor: colors.border, borderRadius: 10, padding: 12, color: colors.text, fontSize: 14, backgroundColor: colors.background, minHeight: 80, textAlignVertical: 'top' }}
                  />
                </View>
              )}
            </ScrollView>
            ...
            <View style={{ padding: 16, borderTopWidth: 1, borderTopColor: colors.border, flexDirection: 'row', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <TouchableOpacity onPress={handleSubmit} disabled={submitting || Object.keys(errors).length > 0 || !isValid} style={{ opacity: submitting || Object.keys(errors).length > 0 || !isValid ? 0.5 : 1, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10, backgroundColor: colors.primary }}>
                <Text style={{ fontWeight: '700', color: '#fff' }}>{submitting ? 'Generating…' : 'Generate Invitation'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </Animated.View>
...
function InvitationSuccessModal({
  visible,
  onClose,
  result,
}: {
  visible: boolean
  onClose: () => void
  result: { method?: string; email?: string; link?: string; code?: string; position?: { name: string } | null } | null
}) {
  const { colors } = useTheme()
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <View style={{ backgroundColor: colors.surface, borderRadius: 18, borderWidth: 1, borderColor: colors.border, width: '100%', maxWidth: 480, padding: 24 }}>
          <View style={{ alignItems: 'center', marginBottom: 16 }}>
            <Check size={48} color={colors.primary} />
          </View>
          <Text style={{ color: colors.text, fontSize: 20, fontWeight: '900', textAlign: 'center', marginBottom: 8 }}>Invitation Created</Text>
          <Text style={{ color: colors.textSecondary, fontSize: 13, textAlign: 'center', marginBottom: 16 }}>Share the invitation with your procurement agent</Text>

          <View style={{ gap: 12 }}>
            <InfoRow label="Method" value={result.method || 'GENERATION'} />
            {result.email && <InfoRow label="Recipient Email" value={result.email} mono />}
            {result.code && <InfoRow label="Code" value={result.code} mono />}
            {result.link && <InfoRow label="Link" value={result.link} mono />}
            {result.position && <InfoRow label="Position" value={result.position.name} />}
          </View>

          <View style={{ flexDirection: 'row', gap: 10, marginTop: 20 }}>
            <TouchableOpacity
              onPress={() => result.link && copyToClipboard(result.link)}
              style={{ flex: 1, backgroundColor: colors.primary, borderRadius: 10, padding: 12, alignItems: 'center', opacity: result.link ? 1 : 0.5 }}
            >
              <Text style={{ color: '#fff', fontWeight: '700', fontSize: 13 }}>Copy Link</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => result.code && copyToClipboard(result.code)}
              style={{ flex: 1, backgroundColor: colors.primary, borderRadius: 10, padding: 12, alignItems: 'center', opacity: result.code ? 1 : 0.5 }}
            >
              <Text style={{ color: '#fff', fontWeight: '700', fontSize: 13 }}>Copy Code</Text>
            </TouchableOpacity>
          </View>

          <TouchableOpacity onPress={onClose} style={{ marginTop: 12, alignSelf: 'center', paddingHorizontal: 20, paddingVertical: 8, borderRadius: 10, borderWidth: 1, borderColor: colors.border }}>
...
export default function HRScreen() {
  ...
  const [hrSectionTab, setHrSectionTab] = useState<HrSectionTab>('employees')
  const [procurementAgents, setProcurementAgents] = useState<OrganizationAgent[]>([])
  ...
  // Organization agent view/edit state
  const [organizationAgent, setOrganizationAgent] = useState<OrganizationAgent | null>(null)
  // Edit mode state for OrganizationAgentModal
  const [agentEditOpen, setAgentEditOpen] = useState(false)
  const [agentEditData, setAgentEditData] = useState<OrganizationAgent | null>(null)
  const [agentEditLoading, setAgentEditLoading] = useState(false)
  // Reject modal state
  const [rejectAgentOpen, setRejectAgentOpen] = useState(false)
  ...
  const [invitationMethod, setInvitationMethod] = useState<'EMAIL' | 'LINK' | 'CODE'>('EMAIL')
  const [inviteEmail, setInviteEmail] = useState('')
  const [invitePositionId, setInvitePositionId] = useState<string | null>(null)
  ...
  const [selectedAgentRequest, setSelectedAgentRequest] = useState<AgentRequestRow | null>(null)
  ...
  // Load procurement agents, invitations, and requests
  const loadProcurement = useCallback(async () => {
    ...
    try {
      const [agents, pendingRequests] = await Promise.all([
        getOrganizationAgents(user.orgId),
        getPendingProcurementAgents(user.orgId),
      ])
      setProcurementAgents(agents ?? [])
      setAgentRequests(pendingRequests ?? [])
    } catch (e: any) {
      if (__DEV__) console.error('Failed to load procurement data:', e)
      setProcurementError(e?.message ?? 'Failed to load procurement data')
    } finally {
  ...
  const refresh = useCallback(async () => {
    setRefreshing(true)
    await Promise.all([load(), loadProcurement()])
    setRefreshing(false)
  ...
  const handleApproveAgent = async (agentId: string,) => {
    try {
      if (__DEV__) console.log('[HRScreen] Approving agent', { agentId })
      await approveOrganizationAgent(agentId)
      if (__DEV__) console.log('[HRScreen] Agent approved', { agentId })
      await loadProcurement()
      toast.show('Agent approved successfully', 'success')
    } catch (e: any) {
      if (__DEV__) console.error('[HRScreen] Approve error', e)
  ...
  const handleRejectAgent = async (agentId: string) => {
    const ok = await confirm({
      title: 'Reject Agent',
      message: 'Are you sure you want to reject this agent request?',
      confirmLabel: 'Reject',
      cancelLabel: 'Cancel',
      destructive: true,
    })
    if (!ok) return
    try {
      if (__DEV__) console.log('[HRScreen] Rejecting agent', { agentId })
      await rejectPendingOrganizationAgent(agentId, 'Rejected by organization')
      if (__DEV__) console.log('[HRScreen] Agent rejected', { agentId })
      await loadProcurement()
      toast.show('Agent rejected', 'success')
    } catch (e: any) {
      if (__DEV__) console.error('[HRScreen] Reject error', e)
      Alert.alert('Error', e?.message ?? 'Failed to reject agent')
    }
  }

  const handleUpdateAgent = async (agentId: string, input: UpdateOrganizationAgentInput) => {
    try {
      if (__DEV__) console.log('[HRScreen] Updating organization agent', { agentId, input })
      await updateOrganizationAgent(agentId, input)
      if (__DEV__) console.log('[HRScreen] Agent updated', { agentId })
      await loadProcurement()
      setAgentEditOpen(false)
      setAgentEditData(null)
      toast.show('Agent updated successfully', 'success')
    } catch (e: any) {
      if (__DEV__) console.error('[HRScreen] Update error', e)
      Alert.alert('Error', e?.message ?? 'Failed to update agent')
    }
  }

  const handleRejectApprovedAgent = async () => {
    ...
    const ok = await confirm({
      ...
      message: `Are you sure you want to reject this approved agent? This will change their verification status to REJECTED and they will no longer appear in Procurement Agents. Reason: ${rejectReason}`,
      confirmLabel: 'Reject',
      cancelLabel: 'Cancel',
      destructive: true,
    })
    if (!ok) return
    try {
      if (__DEV__) console.log('[HRScreen] Rejecting approved agent', { agentId: rejectAgentId, reason: rejectReason })
      await rejectApprovedOrganizationAgent(rejectAgentId, rejectReason.trim())
      if (__DEV__) console.log('[HRScreen] Approved agent rejected', { agentId: rejectAgentId })
      setRejectAgentOpen(false)
      setRejectAgentId(null)
      setRejectReason('')
      await loadProcurement()
      toast.show('Agent rejected successfully', 'success')
    } catch (e: any) {
      if (__DEV__) console.error('[HRScreen] Reject approved agent error', e)
      Alert.alert('Error', e?.message ?? 'Failed to reject agent')
    }
  }
  ...
  const handleResendInvitation = async (id: string) => {
    try {
      await HrService.resendInvitation(id)
      await loadProcurement()
    } catch (e: any) {
      Alert.alert('Error', e?.message ?? 'Failed to resend invitation')
    }
  }
  ...
  const handleRevokeInvitation = async (id: string) => {
    const ok = await confirm({
      ...
      message: 'This invitation will immediately become invalid.',
    ...
    if (ok) {
      try {
        await HrService.revokeInvitation(id)
        await loadProcurement()
      } catch (e: any) {
  ...
  const handleInvitationGenerated = (result: any) => {
    // Immediately add to invitations list and open success modal
  ...
  const handleExtendExpiration = async (id: string) => {
    const ok = await confirm({
      ...
      message: 'This will add 7 days to the invitation expiry.',
      confirmLabel: 'Extend',
      cancelLabel: 'Cancel',
    })
    if (ok) {
      try {
        await HrService.extendInvitationExpiration(id, 7)
        await loadProcurement()
        Alert.alert('Success', 'Expiration extended by 7 days')
      } catch (e: any) {
        Alert.alert('Error', e?.message ?? 'Failed to extend expiration')
      }
    }
  }
  ...
  const handleDeleteInvitation = async (id: string) => {
    const ok = await confirm({
      ...
      message: 'This invitation will be permanently removed.',
    ...
    if (ok) {
      try {
        await HrService.deleteInvitation(id)
        setInvitations(prev => prev.filter(inv => inv.id !== id))
      } catch (e: any) {
        Alert.alert('Error', e?.message ?? 'Failed to delete invitation')
      }
    }
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        ...
        {loading || procurementLoading ? <LoadingRows /> : error ? (
          ...
          <>
            ...
            <TabBar
              tabs={[
                ...
                { key: 'procurementAgents', label: 'Procurement Agents', icon: UserCheck },
                { key: 'invitations', label: 'Invitations', icon: Mail },
                { key: 'pendingRequests', label: 'Pending Requests', icon: Shield },
              ]}
              active={hrSectionTab}
              onChange={setHrSectionTab}
              scrollable
            />

            {/* Employees Section */}
            {hrSectionTab === 'employees' && (
              <>
                {/* KPI Stats */}
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
            ...
            {/* Procurement Agents Section */}
            {hrSectionTab === 'procurementAgents' && (
              <>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                  <StatCard title="Total Agents" value={procurementAgents.length} subtitle="All procurement agents" accent="#0EA5E9" icon={UserCheck} widthPct={isDesktop ? '23%' : isTablet ? '45%' : '100%'} />
                  <StatCard title="Active" value={procurementAgents.filter(a => a.status === 'ACCEPTED').length} subtitle="Accepted invitations" accent="#16A34A" icon={UserCheck} widthPct={isDesktop ? '23%' : isTablet ? '45%' : '100%'} />
                  <StatCard title="Inactive" value={procurementAgents.filter(a => a.status !== 'ACCEPTED').length} subtitle="Non-active agents" accent="#6B7280" icon={ZapOff} widthPct={isDesktop ? '23%' : isTablet ? '45%' : '100%'} />
                  <StatCard title="Positions" value={positions.length} subtitle="Available roles" accent="#7C3AED" icon={ShieldCheck} widthPct={isDesktop ? '23%' : isTablet ? '45%' : '100%'} />
                </View>

                <SectionCard
                  title={`Procurement Agents  ·  ${procurementAgents.length} agents`}
                  subtitle="Active agents assigned to procurement operations"
                  padded={false}
                >
                  <View style={{ padding: 16 }}>
                    {procurementLoading ? (
                      ...
                      <View style={{ alignItems: 'center', paddingVertical: 40, gap: 12 }}>
                        <Text style={{ color: colors.textSecondary, fontSize: 14 }}>Failed to load procurement agents</Text>
                        <TouchableOpacity onPress={() => loadProcurement()} style={{ backgroundColor: colors.primary, borderRadius: 10, paddingHorizontal: 16, paddingVertical: 10 }}>
                          <Text style={{ color: '#fff', fontWeight: '700', fontSize: 13 }}>Retry</Text>
                        </TouchableOpacity>
                      </View>
                    ) : procurementAgents.length === 0 ? (
                      <EmptyState icon={UserCheck} title="No approved procurement agents" message="Approved agents assigned to your organization will appear here." />
                    ) : (
                      <ProcurementAgentTable
                        agents={procurementAgents}
                        onView={(agent) => {
                          if (__DEV__) console.log('[HRScreen] Viewing organization agent', { agentId: agent.id })
                          setOrganizationAgent(agent)
                          setAgentDetailsOpen(true)
                        }}
                        onEdit={(agent) => {
                          if (__DEV__) console.log('[HRScreen] Editing organization agent', { agentId: agent.id })
                          setAgentEditData(agent)
                          setAgentEditOpen(true)
                        }}
                        onReject={(agent) => {
                          if (__DEV__) console.log('[HRScreen] Rejecting organization agent', { agentId: agent.id })
                          setRejectAgentId(agent.id)
                          setRejectReason('')
                          setRejectAgentOpen(true)
                        }}
                      />
                    )}
                  </View>
            ...
            {/* Invitations Section */}
            {hrSectionTab === 'invitations' && (
              <>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
                  <StatCard title="Total" value={invitations.length} subtitle="All invitations" accent="#0EA5E9" icon={Mail} widthPct={isDesktop ? '23%' : isTablet ? '45%' : '100%'} />
                  <StatCard title="Pending" value={invitations.filter(i => i.status === 'PENDING').length} subtitle="Awaiting acceptance" accent="#F59E0B" icon={Clock} widthPct={isDesktop ? '23%' : isTablet ? '45%' : '100%'} />
                  <StatCard title="Accepted" value={invitations.filter(i => i.status === 'ACCEPTED').length} subtitle="Activated" accent="#16A34A" icon={UserCheck} widthPct={isDesktop ? '23%' : isTablet ? '45%' : '100%'} />
                  <TouchableOpacity onPress={handleGenerateInvitation} style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: colors.primary, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8 }}>
                    <Plus size={14} color="#fff" />
                    <Text style={{ color: '#fff', fontWeight: '700', fontSize: 13 }}>Generate Invitation</Text>
                  </TouchableOpacity>
                </View>

                <SectionCard
                  title={`Invitations  ·  ${invitations.length} invitations`}
                  subtitle="Procurement agent invitation codes and links"
                  padded={false}
                >
                  <View style={{ padding: 16 }}>
                    {invitations.length === 0 ? (
                      <EmptyState icon={Mail} title="No invitations yet" message="Generate invitation links or codes to invite procurement agents." />
                    ) : (
                      <InvitationTable
            ...
            {hrSectionTab === 'pendingRequests' && (
              <>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                  <StatCard title="Total Pending" value={agentRequests.length} subtitle="All requests" accent="#0EA5E9" icon={Shield} widthPct={isDesktop ? '30%' : isTablet ? '45%' : '100%'} />
                  <StatCard title="Pending Approval" value={agentRequests.filter(r => r.status === 'PENDING').length} subtitle="Awaiting review" accent="#F59E0B" icon={Clock} widthPct={isDesktop ? '30%' : isTablet ? '45%' : '100%'} />
                  <StatCard title="Positions" value={positions.length} subtitle="Available roles" accent="#7C3AED" icon={ShieldCheck} widthPct={isDesktop ? '30%' : isTablet ? '45%' : '100%'} />
                </View>

                <SectionCard
                  title={`Pending Requests  ·  ${agentRequests.length} requests`}
                  subtitle="Procurement agent join requests awaiting approval"
                  padded={false}
                >
                  <View style={{ padding: 16 }}>
                    {procurementLoading ? (
                      ...
                      <View style={{ alignItems: 'center', paddingVertical: 40, gap: 12 }}>
                        <Text style={{ color: colors.textSecondary, fontSize: 14 }}>Failed to load pending requests</Text>
                        <TouchableOpacity onPress={loadProcurement} style={{ backgroundColor: colors.primary, borderRadius: 10, paddingHorizontal: 16, paddingVertical: 10 }}>
                          <Text style={{ color: '#fff', fontWeight: '700', fontSize: 13 }}>Retry</Text>
                        </TouchableOpacity>
                      </View>
                    ) : agentRequests.length === 0 ? (
                      <EmptyState icon={Shield} title="No pending requests" message="No procurement agents have requested to join your organization." />
                    ) : (
                      <PendingRequestsTable
                        requests={agentRequests}
                        onApprove={handleApproveAgent}
                        onReject={handleRejectAgent}
                        onViewDetails={async (agentId) => {
                          if (__DEV__) console.log('[HRScreen] Viewing agent details', { agentId })
                          try {
                            const details = await getOrganizationAgentDetails(agentId)
                            if (!details) {
                              Alert.alert('Error', 'No details found for this agent.')
                              return
                            }
                            setAgentDetails(details)
                            setAgentDetailsOpen(true)
                          } catch (e: any) {
                            Alert.alert('Error', e?.message ?? 'Failed to load agent details')
                          }
                        }}
                      />
                    )}
            ...
            {/* Position Management - Only show in employees tab */}
            {hrSectionTab === 'employees' && (
              <SectionCard
      ...
      <InvitationDrawer
        visible={invitationDrawerOpen}
        positions={positions}
        onClose={() => setInvitationDrawerOpen(false)}
      ...
      />
      {/* Pending Agent Modal (for pending requests) */}
      <PendingAgentModal
        visible={agentDetailsOpen && agentDetails !== null && organizationAgent === null}
        agent={agentDetails}
        onClose={() => { setAgentDetailsOpen(false); setAgentDetails(null) }}
        onApprove={async () => {
          if (agentDetails?.id) {
            await handleApproveAgent(agentDetails.id)
            await loadProcurement()
          }
        }}
        onReject={async () => {
          if (agentDetails?.id) {
            await handleRejectAgent(agentDetails.id)
            await loadProcurement()
          }
        }}
        onRefresh={loadProcurement}
      />

      {/* Organization Agent View Modal */}
      {agentDetailsOpen && organizationAgent && !agentEditOpen && (
        <OrganizationAgentModal
          visible={agentDetailsOpen && organizationAgent !== null}
          agent={organizationAgent}
          mode="view"
          onClose={() => { setAgentDetailsOpen(false); setOrganizationAgent(null) }}
          onSave={handleUpdateAgent}
          onReject={async (agentId, reason) => {
            await rejectApprovedOrganizationAgent(agentId, reason)
            await loadProcurement()
          }}
        />
      )}

      {/* Organization Agent Edit Modal */}
      {agentEditOpen && agentEditData && (
        <OrganizationAgentModal
          visible={agentEditOpen}
          agent={agentEditData}
          mode="edit"
          onClose={() => { setAgentEditOpen(false); setAgentEditData(null) }}
          onSave={handleUpdateAgent}
        />
      )}

      {/* Reject Approved Agent Confirmation Dialog */}
      <Modal
        visible={rejectAgentOpen}
        transparent
        animationType="fade"
        onRequestClose={() => { setRejectAgentOpen(false); setRejectReason('') }}
      >
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <TouchableOpacity style={{ position: 'absolute', inset: 0 }} onPress={() => { setRejectAgentOpen(false); setRejectReason('') }} activeOpacity={1} />
          <View style={{ backgroundColor: colors.surface, borderRadius: 20, padding: 24, width: '90%', maxWidth: 480, borderWidth: 1, borderColor: colors.border }}>
            <Text style={{ fontSize: 18, fontWeight: '900', color: colors.text, marginBottom: 8 }}>Reject Approved Agent</Text>
            <Text style={{ fontSize: 13, color: colors.textSecondary, marginBottom: 16 }}>
              This agent will be marked as REJECTED and will immediately disappear from the Procurement Agents list. This action cannot be undone.
            </Text>
            <TextInput
            ...
            <View style={{ flexDirection: 'row', gap: 10, justifyContent: 'flex-end' }}>
              <TouchableOpacity
                onPress={() => { setRejectAgentOpen(false); setRejectReason('') }}
                style={{ paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10, backgroundColor: colors.background, borderWidth: 1, borderColor: colors.border }}
              >
                <Text style={{ color: colors.text, fontWeight: '700', fontSize: 13 }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={handleRejectApprovedAgent}
                disabled={rejectLoading || !rejectReason.trim()}
                style={{
                  paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10, backgroundColor: '#DC2626',
                  opacity: rejectLoading || !rejectReason.trim() ? 0.6 : 1,
                }}
              >
                <Text style={{ color: '#fff', fontWeight: '700', fontSize: 13 }}>
                  {rejectLoading ? 'Rejecting...' : 'Reject Agent'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
